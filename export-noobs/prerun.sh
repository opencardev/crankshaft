...
BOOT_DEV=$(run_flappy 'losetup --show -v -f -o "${BOOT_OFFSET}" --sizelimit "${BOOT_LENGTH}" /dev/loop6 "${IMG_FILE}"')

ROOT_DEV=$(run_flappy 'losetup --show -v -f -o "${ROOT_OFFSET}" --sizelimit "${ROOT_LENGTH}" /dev/loop4 "${IMG_FILE}"')
